...
import ClientData from "./EnterClientData";
...
export default function Client() {
  ...
  const handleDelete = async (id) => {
    ...
    if (!confirmed) return;
    try {
      await axios.delete(`http://localhost:5000/api/deleteClient/${id}`);
  ...
  const handleUpdate = async (id) => {
    try {
      await axios.put(`http://localhost:5000/api/updateClient/${id}`, editData);
